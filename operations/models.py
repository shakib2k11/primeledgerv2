...
class TradeDocument(models.Model):
    ...
    def clean(self):
        if self.number and (len(self.number) != 8 or not self.number.isdigit()):
            raise ValidationError({"number": _("The document number must contain exactly eight digits.")})
        related = ("party", "period", "debit_account", "credit_account")
        for field in related:
            value = getattr(self, field, None)
            if value and value.business_id != self.business_id:
                raise ValidationError(_("Document relationships must belong to the same business."))
        if self.period_id and not self.period.starts_on <= self.document_date <= self.period.ends_on:
            raise ValidationError(_("Document date must fall within its fiscal period."))
        ...
        if self.kind == self.Kind.PURCHASE and (
            ...
            raise ValidationError(_("Discounts are available on sales only."))
        if self.discount_type == self.DiscountType.NONE and self.discount_value:
            raise ValidationError({"discount_value": _("Choose a discount type before entering a value.")})
        if (
            self.discount_type == self.DiscountType.PERCENTAGE
            and self.discount_value >= Decimal("100")
        ):
            raise ValidationError({"discount_value": _("Percentage discount must be less than 100.")})
    ...
    def set_totals(self, subtotal):
        ...
        if discount >= subtotal and discount > 0:
            raise ValidationError({
                "discount_value": _("Discount must be less than the sale subtotal.")
            })
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            if previous and previous["number"] != self.number:
                raise ValidationError(_("The automatic document number cannot be changed."))
            if previous and previous["status"] == self.Status.POSTED:
                raise ValidationError(_("Posted sales and purchases cannot be edited."))
        super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        if self.status == self.Status.POSTED:
            raise ValidationError(_("Posted sales and purchases cannot be deleted."))
        if self.period.is_locked:
            raise ValidationError(_("Documents in a locked fiscal period cannot be deleted."))
...
class TradeLine(models.Model):
    ...
    def clean(self):
        if self.document_id and self.product_id and self.document.business_id != self.product.business_id:
            raise ValidationError(_("Line product must belong to the document business."))

    def save(self, *args, **kwargs):
        if self.document_id and self.document.status == TradeDocument.Status.POSTED:
            raise ValidationError(_("Lines in posted documents cannot be edited."))
    ...
    def delete(self, *args, **kwargs):
        if self.document.status == TradeDocument.Status.POSTED:
            raise ValidationError(_("Lines in posted documents cannot be deleted."))
        return super().delete(*args, **kwargs)
...
class SalePayment(models.Model):
    ...
    def clean(self):
        ...
        if self.number and (len(self.number) != 8 or not self.number.isdigit()):
            raise ValidationError({
                "number": _("The payment number must contain exactly eight digits.")
            })
        for field in ("sale", "payment_account", "journal_entry", "money_receipt"):
            value = getattr(self, field, None)
            if value and value.business_id != self.business_id:
                raise ValidationError(_("Payment relationships must belong to the same business."))
        if self.sale_id and (
            self.sale.kind != TradeDocument.Kind.SALE
            or self.sale.status != TradeDocument.Status.POSTED
        ):
            raise ValidationError(_("Payments require a posted sale."))
        if self.sale_id and (
            self.sale.debit_account.system_role
            != Account.SystemRole.ACCOUNTS_RECEIVABLE
        ):
            raise ValidationError(
                _("Payments can be allocated only to a sale posted to Accounts Receivable.")
            )
        ...
        if self.journal_entry_id and not self.journal_entry.posted:
            raise ValidationError(_("A sale payment requires a posted journal entry."))
        if self.money_receipt_id:
            ...
            if self.money_receipt.receipt_date != self.payment_date:
                raise ValidationError(_("Payment and money receipt dates must match."))
            if (
                self.money_receipt.voucher.journal_entry_id
                != self.journal_entry_id
            ):
                raise ValidationError(_("Payment receipt must use the payment journal entry."))

    def save(self, *args, **kwargs):
        if self.pk:
            raise ValidationError(_("Posted sale payments cannot be edited."))
        super().save(*args, **kwargs)
...
class PurchasePayment(models.Model):
    ...
    def clean(self):
        ...
        if self.purchase_id and (
            ...
            raise ValidationError(
                _("Payments can be allocated only to a purchase posted to Accounts Payable.")
            )
        if self.payment_account_id and self.payment_account.system_role not in liquid_roles:
            raise ValidationError(
                _("Payment account must be mapped as Cash, Bank, or Mobile Financial Services.")
...
class BalanceSetoff(models.Model):
    ...
    def clean(self):
        if self.number and (len(self.number) != 8 or not self.number.isdigit()):
            raise ValidationError({
                "number": _("The set-off number must contain exactly eight digits.")
            })
        ...
        if self.voucher_id:
            if self.voucher.voucher_type != Voucher.Type.CONTRA:
                raise ValidationError(_("Balance set-off requires a contra voucher."))
            ...
            if self.voucher.journal_entry_id != self.journal_entry_id:
                raise ValidationError(_("Contra voucher must use the set-off journal entry."))

    def save(self, *args, **kwargs):
        if self.pk:
            raise ValidationError(_("Posted balance set-offs cannot be edited."))
        super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        raise ValidationError(_("Posted balance set-offs cannot be deleted."))
    ...
    def clean(self):
        if self.setoff_id and self.sale_id:
            ...
            if (
                ...
                raise ValidationError(_("Set-off requires a posted Accounts Receivable sale."))

    def save(self, *args, **kwargs):
        if self.pk:
            raise ValidationError(_("Set-off allocations cannot be edited."))
        super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        raise ValidationError(_("Set-off allocations cannot be deleted."))
    ...
    def clean(self):
        if self.setoff_id and self.purchase_id:
            ...
            if (
                ...
                raise ValidationError(_("Set-off requires a posted Accounts Payable purchase."))
